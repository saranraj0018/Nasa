    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach ($upcomingEvents as $event)
            ...
            <div class="bg-white rounded-2xl shadow hover:shadow-lg transition">
                <div class="relative">
                    <img src="{{ asset('storage/' . $event['banner_image']) }}" alt="Event" class="w-full h-48 object-cover rounded-t-2xl">
                ...
                <div class="p-2 details">
                    <div class="grid grid-cols-1 gap-1 md:grid-cols-4 text-xs">
                        <div class="col-span-2 flex items-center bg-[#F2E8F5] rounded-full px-1 py-1">
                            ...
                            <p class="px-1">{{ $event->start_time ? \Carbon\Carbon::parse($event->start_time)->format('h:iA') : '-' }} - {{ $event->end_time ? \Carbon\Carbon::parse($event->end_time)->format('h:iA') : '-' }}</p>
                        </div>
                        <div class="col-span-2 flex items-center bg-[#F2E8F5] rounded-full px-2 py-1">
                            ...
                            <p class="px-1">{{ \Carbon\Carbon::parse($event['event_date'])->format('F j, Y') }}</p>
                        </div>
...
<div id="ongoing-section" class="mt-6 hidden">
    ...
    $today = \Carbon\Carbon::now();
            ...
            <div class="bg-white rounded-2xl shadow hover:shadow-lg transition">
                <div class="relative">
                    <img src="{{ asset('storage/' . $ongoing_event['banner_image']) }}" alt="Event" class="w-full h-48 object-cover rounded-t-2xl">
                ...
                <div class="p-2 details">
                    <div class="grid grid-cols-1 gap-1 md:grid-cols-4 text-xs">
                        <div class="col-span-2 flex items-center bg-[#F2E8F5] rounded-full px-1 py-1">
                            <i class="fa fa-clock text-primary" aria-hidden="true"></i>
                            <p class="px-1">{{ $ongoing_event->start_time ? \Carbon\Carbon::parse($ongoing_event->start_time)->format('h:iA') : '-' }} - {{ $ongoing_event->end_time ? \Carbon\Carbon::parse($ongoing_event->end_time)->format('h:iA') : '-' }}</p>
                        </div>
                        <div class="col-span-2 flex items-center bg-[#F2E8F5] rounded-full px-2 py-1">
                            <i class="fa fa-calendar text-primary" aria-hidden="true"></i>
                            <p class="px-1">{{ \Carbon\Carbon::parse($ongoing_event['event_date'])->format('F j, Y') }}</p>
                        </div>
                        <div class="col-span-4 flex items-center bg-[#F2E8F5] rounded-full px-1 py-1 mt-2">
                           <i class="fa fa-map-marker text-primary" aria-hidden="true"></i>
